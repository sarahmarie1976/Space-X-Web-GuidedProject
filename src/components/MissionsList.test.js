...
import MissionsList from './MissionsList';
    ...
    /* test function - global function coming from our JEST library
     it gonna the namae of the test and a callback function in charge of running our test */
...
test("Re-Renders correctly with list of missions", () => {
            // arrange (render the component and set up mock data )
                /* render(<MissionsList />); this brings up an error because it cannot read the property map
        	         because  it is undefined
        	        pass the missions prop as an empty array
                    setup mock data */ 

                   
    const { rerender, getAllByTestId, queryAllByTestId } = render(<MissionsList missions={[]}  />);  
    
            // assert that there are no missions yet
    ...
    const missions = getAllByTestId(/mission/i); // getAll Functions return an array if it finds multiple elements
...
test("displays an error message if an error is passed in", () => {
    ...
    // re-render the component _with_ an error passed down
